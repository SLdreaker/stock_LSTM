...
data_open_n = data_normal[:1260,2]
...
with session as sess:
    ...
    saver.restore(session, '/home/xxxxxx/Final_model_lstm_low')
    
    
    train_batches = get_batches(data_open_n, batch_size, num_steps)
    for next_time in range(0,start/50+1):
...
np.save("DayLow.npy",re_)
plt.plot(re_, label='predict')
plt.plot(data[start+num_steps:start+max_steps+num_steps,2], label='real')
plt.legend(loc='upper right')
plt.title('Yuan Da Taiwan 50- Day Low')
plt.xlabel('day')
plt.ylabel('price')
plt.show()
